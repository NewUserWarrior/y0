...
exports.run = async (client, message, args) => {
  ...
  if(!kwejn) return message.reply(`:x: Lütfen bir kullanıcı adı giriniz! \nDoğru Kullanım; \`${prefix}isimdeğiştir @${client.user.username}#${client.user.discriminator} \``)

  if(kwejn.length > 32) return message.reply(`:x: Lütfen \`32\` karakteri geçmeyecek şekilde bir isim giriniz!`)
  message.guild.members.cache.get(kullanici.id).setNickname(`${kwejn}`)

  message.channel.send(`:white_check_mark: Başarılı bir şekilde \`${kullanici.username}\` adlı kişinin kullanıcı adı \`${kwejn}\` olarak değiştirildi.`)
}
exports.conf = {
    enabled: true,
    guildOnly: false,
